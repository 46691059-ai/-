param(
    [string]$OutputRoot = "D:\codex-validation-v266-$((Get-Date).ToString('yyyyMMdd-HHmmss'))"
)
...
try {
    ...
    $upgrade = New-IsolatedMysql 'upgrade-v265' 34866
    Initialize-Baseline $fresh; Initialize-Baseline $upgrade

    Invoke-Flyway $fresh migrate '2.6.6'; Invoke-Flyway $fresh validate; Invoke-Flyway $fresh migrate '2.6.6'
    # V2.6.5 is only an Upgrade-path preparation point. Strict validation is
    # intentionally performed after V2.6.6 because the full scan root already
    # contains the pending candidate and Flyway 13 treats that interim state as invalid.
    Invoke-Flyway $upgrade migrate '2.6.5'
    Invoke-Flyway $upgrade migrate '2.6.6'; Invoke-Flyway $upgrade validate; Invoke-Flyway $upgrade migrate '2.6.6'
...
"@
    ...
    $checksumFresh = (Invoke-Mysql $fresh "SELECT checksum FROM flyway_schema_history WHERE version='2.6.6' AND success=1" -Raw).Output.Trim()
    $checksumUpgrade = (Invoke-Mysql $upgrade "SELECT checksum FROM flyway_schema_history WHERE version='2.6.6' AND success=1" -Raw).Output.Trim()
    if ($checksumFresh -ne $checksumUpgrade) { throw 'V2.6.6 Flyway checksums differ' }
    ...
    $summary = [ordered]@{
        ...
        v266_sha256=(Get-FileHash (Join-Path $repoMigrations 'V2.6.6__create_workflow_candidate_pool.sql') -Algorithm SHA256).Hash.ToLower();
        v266_flyway_checksum=$checksumFresh; fresh_history_success=[int](Invoke-Mysql $fresh "SELECT COUNT(*) FROM flyway_schema_history WHERE success=1" -Raw).Output.Trim();
