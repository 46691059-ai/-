...
-- any target object already exists, or a PERSISTED activation does not carry the full
-- Request -> three APPROVE decisions -> five evidence records chain. PERSISTED has the
-- frozen semantic "complete activation approval evidence chain persisted"; status alone
-- is never accepted as proof. No data is repaired, inferred or deleted.
CREATE TEMPORARY TABLE tmp_role_binding_v2614_guard (
    phase_code VARCHAR(32) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    violation_count BIGINT NOT NULL,
    PRIMARY KEY (phase_code),
    CONSTRAINT chk_role_binding_v2614_guard CHECK (violation_count = 0)
);

-- Phase A: immutable parent contract, clean target namespace and valid PERSISTED requests.
INSERT INTO tmp_role_binding_v2614_guard (phase_code, violation_count)
SELECT
    'PHASE_A_REQUEST',
  ...
  + (SELECT COUNT(*) FROM role_runtime_activation_request
       WHERE status = 'PERSISTED'
         AND (resolver_code IS NULL OR resolver_code = ''
           ...
           OR directory_revision < 0
           OR CHAR_LENGTH(TRIM(business_scope)) = 0));

-- Phase B: every PERSISTED request must own the complete RACI approval and evidence set.
INSERT INTO tmp_role_binding_v2614_guard (phase_code, violation_count)
SELECT 'PHASE_B_COMPLETENESS', COUNT(*)
  FROM role_runtime_activation_request r
 WHERE r.status = 'PERSISTED'
   AND (
       (SELECT COUNT(*) FROM role_runtime_activation_approval a
         WHERE a.activation_id = r.activation_id AND a.delete_token = r.delete_token) <> 3
    OR (SELECT COUNT(DISTINCT a.approver_type) FROM role_runtime_activation_approval a
         WHERE a.activation_id = r.activation_id AND a.delete_token = r.delete_token
           AND a.decision = 'APPROVE') <> 3
    OR NOT EXISTS (SELECT 1 FROM role_runtime_activation_approval a
                    WHERE a.activation_id = r.activation_id AND a.delete_token = r.delete_token
                      AND a.approver_type = 'BUSINESS_OWNER' AND a.decision = 'APPROVE')
    OR NOT EXISTS (SELECT 1 FROM role_runtime_activation_approval a
                    WHERE a.activation_id = r.activation_id AND a.delete_token = r.delete_token
                      AND a.approver_type = 'SECURITY_AUDIT' AND a.decision = 'APPROVE')
    OR NOT EXISTS (SELECT 1 FROM role_runtime_activation_approval a
                    WHERE a.activation_id = r.activation_id AND a.delete_token = r.delete_token
                      AND a.approver_type = 'RELEASE_APPROVER' AND a.decision = 'APPROVE')
    OR (SELECT COUNT(*) FROM role_runtime_activation_evidence e
         WHERE e.activation_id = r.activation_id AND e.delete_token = r.delete_token) <> 5
    OR (SELECT COUNT(DISTINCT e.evidence_type) FROM role_runtime_activation_evidence e
         WHERE e.activation_id = r.activation_id AND e.delete_token = r.delete_token) <> 5
    OR NOT EXISTS (SELECT 1 FROM role_runtime_activation_evidence e
                    WHERE e.activation_id = r.activation_id AND e.delete_token = r.delete_token
                      AND e.evidence_type = 'ACTIVATION_APPROVAL'
                      AND e.evidence_hash = r.approval_evidence_hash)
    OR NOT EXISTS (SELECT 1 FROM role_runtime_activation_evidence e
                    WHERE e.activation_id = r.activation_id AND e.delete_token = r.delete_token
                      AND e.evidence_type = 'RESOLVER_CONTRACT'
                      AND e.evidence_hash = r.contract_hash)
    OR NOT EXISTS (SELECT 1 FROM role_runtime_activation_evidence e
                    WHERE e.activation_id = r.activation_id AND e.delete_token = r.delete_token
                      AND e.evidence_type = 'BINDING'
                      AND e.evidence_hash = r.binding_hash)
    OR NOT EXISTS (SELECT 1 FROM role_runtime_activation_evidence e
                    WHERE e.activation_id = r.activation_id AND e.delete_token = r.delete_token
                      AND e.evidence_type = 'CANDIDATE'
                      AND e.evidence_hash = r.candidate_hash)
    OR NOT EXISTS (SELECT 1 FROM role_runtime_activation_evidence e
                    WHERE e.activation_id = r.activation_id AND e.delete_token = r.delete_token
                      AND e.evidence_type = 'DIRECTORY'
                      AND e.evidence_hash = r.directory_contract_hash)
   );

-- Phase C: child ownership, decision state and all stored hash contracts must agree.
INSERT INTO tmp_role_binding_v2614_guard (phase_code, violation_count)
SELECT 'PHASE_C_INTEGRITY',
    (SELECT COUNT(*) FROM role_runtime_activation_approval a
      ...
      WHERE r.id IS NULL OR a.activation_hash <> r.activation_hash
         ...
         OR (r.status = 'PERSISTED' AND a.decision <> 'APPROVE'))
  + (SELECT COUNT(*) FROM role_runtime_activation_evidence e
       ...
       LEFT JOIN role_runtime_activation_request r
         ON r.activation_id = e.activation_id AND r.delete_token = e.delete_token
      WHERE a.id IS NULL OR r.id IS NULL OR e.activation_hash <> a.activation_hash
         OR e.contract_hash <> a.contract_hash OR e.binding_hash <> a.binding_hash
         OR e.activation_hash <> r.activation_hash
         OR e.contract_hash <> r.contract_hash OR e.binding_hash <> r.binding_hash
         OR (r.status = 'PERSISTED' AND a.decision <> 'APPROVE'));

-- Phase D is reached only if every prior CHECK succeeded; permanent DDL starts afterwards.
INSERT INTO tmp_role_binding_v2614_guard (phase_code, violation_count)
VALUES ('PHASE_D_COMPLETE', 0);
