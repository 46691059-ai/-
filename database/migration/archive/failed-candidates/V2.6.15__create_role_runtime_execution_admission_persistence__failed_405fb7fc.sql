...
SELECT 'NO_PARTIAL_INSTALL',
     ...
     + (SELECT COUNT(*) FROM information_schema.statistics
         WHERE table_schema=DATABASE() AND index_name LIKE '%role_admission%');
...
CREATE TABLE workflow_role_runtime_execution_admission (
    ...
    CONSTRAINT fk_role_admission_candidate FOREIGN KEY
    ...
    CONSTRAINT fk_role_admission_definition_version FOREIGN KEY (definition_id,definition_version_id)
      REFERENCES workflow_version(definition_id,id) ON DELETE RESTRICT ON UPDATE RESTRICT,
    CONSTRAINT fk_role_admission_node FOREIGN KEY (definition_version_id,node_id)
      REFERENCES workflow_node(version_id,id) ON DELETE RESTRICT ON UPDATE RESTRICT,
    CONSTRAINT fk_role_admission_release FOREIGN KEY (definition_release_id)
      REFERENCES workflow_version_release(id) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Phase 4: append-only validator and capability evidence.
CREATE TABLE workflow_role_runtime_execution_admission_evidence (
    ...
    CONSTRAINT fk_role_admission_evidence_owner FOREIGN KEY (admission_row_id,admission_id,delete_token)
      REFERENCES workflow_role_runtime_execution_admission(id,admission_id,delete_token) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Phase 5: append-only lifecycle hash chain.
CREATE TABLE workflow_role_runtime_execution_admission_event (
    ...
    CONSTRAINT fk_role_admission_event_owner FOREIGN KEY (admission_row_id,admission_id,candidate_snapshot_row_id,delete_token)
...
CREATE TABLE workflow_role_runtime_execution_admission_slot (
    ...
    CONSTRAINT fk_role_admission_slot_candidate FOREIGN KEY
      ...
      ON DELETE RESTRICT ON UPDATE RESTRICT,
    CONSTRAINT fk_role_admission_slot_active FOREIGN KEY
      (active_admission_row_id,active_admission_id,candidate_snapshot_row_id,source_delete_token)
      REFERENCES workflow_role_runtime_execution_admission(id,admission_id,candidate_snapshot_row_id,delete_token)
      ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Phase 7: ownership and append-only guards. Stable SQLSTATE 45000 messages are audit contracts.
DELIMITER $$
...
-- Phase 8: static final structure assertion after all permanent DDL.
