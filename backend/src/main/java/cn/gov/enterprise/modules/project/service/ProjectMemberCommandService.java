...
public class ProjectMemberCommandService {
    ...
    public void delete(Long projectId, Long memberId) {
        ...
        ProjectMemberEntity member = requireMember(projectId, memberId);
        if (member.getUserId().equals(project.getManagerUserId())
                && "MANAGER".equals(member.getMemberRole())) {
            throw new BusinessException("B0001", "项目负责人不能从成员中移除");
        }
        if (memberMapper.softDelete(memberId, securityContext.userId()) == 0) {
            throw concurrentModification();
        }
    }
    ...
    public void synchronizeManager(
            Long projectId, Long previousUserId, Long userId, LocalDate joinedDate) {
        if (previousUserId != null && !previousUserId.equals(userId)) {
            ProjectMemberEntity previous = memberMapper.selectOne(
                    new LambdaQueryWrapper<ProjectMemberEntity>()
                            ...
                            .eq(ProjectMemberEntity::getUserId, previousUserId)
                            .eq(ProjectMemberEntity::getMemberRole, "MANAGER"));
            if (previous != null) {
                previous.setMemberStatus("INACTIVE");
                previous.setLeftDate(LocalDate.now());
                if (memberMapper.updateById(previous) == 0) {
                    throw concurrentModification();
                }
            }
        }
        ProjectMemberEntity current = memberMapper.selectOne(
                new LambdaQueryWrapper<ProjectMemberEntity>()
                        .eq(ProjectMemberEntity::getProjectId, projectId)
    ...
    private void ensureUnique(
        ...
        long count = memberMapper.selectCount(new LambdaQueryWrapper<ProjectMemberEntity>()
                .eq(ProjectMemberEntity::getProjectId, projectId)
                .eq(ProjectMemberEntity::getUserId, request.userId())
                .eq(ProjectMemberEntity::getMemberRole, request.memberRole())
                .ne(memberId != null, ProjectMemberEntity::getId, memberId));
        if (count > 0) {
            throw new BusinessException("B0001", "该用户已具有相同项目角色");
        }
    }
    ...
    private void validate(ProjectDtos.MemberRequest request) {
        if (request.leftDate() != null && request.joinedDate().isAfter(request.leftDate())) {
            throw new BusinessException("B0001", "成员参与日期的开始日期不能晚于结束日期");
        }
        referenceValidator.requireActiveUser(request.userId(), "项目成员");
    }
